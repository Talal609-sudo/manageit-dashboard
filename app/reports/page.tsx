  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        {/* Orders by category (horizontal bars, no overlap) */}
        <div className="card p-4">
          ...
          <ul className="space-y-3" aria-label="Orders by category">
            {([
              ['Jackets', 88, 'bg-teal-500'] as const,
              ['Wallets', 64, 'bg-purple-500'] as const,
              ['Bags', 52, 'bg-blue-500'] as const,
              ['Hoodies', 40, 'bg-amber-500'] as const,
            ]).map(([name, val, cls]) => {
              const widthPct = (val / 88) * 100; // 88 is max here
              return (
                <li key={name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-6 rounded-full bg-black/5 dark:bg-white/10 relative overflow-hidden">
                    <div
                      className={`h-6 ${cls} rounded-full`}
